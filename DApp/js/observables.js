...
const walletChoosingObservable = new Observable(subscriber => {
    onProviderSearchingByKeyNextTry(subscriber, METAMASK_KEY)
        .then(provider => onWaitingForWalletChoice(subscriber, provider, METAMASK_KEY, $('[id=metamaskChoosingButton]')))
    onProviderSearchingByKeyNextTry(subscriber, BINANCE_WALLET_KEY)
        .then(provider => onWaitingForWalletChoice(subscriber, provider, BINANCE_WALLET_KEY, $('[id=binanceWalletChoosingButton]')))
})
